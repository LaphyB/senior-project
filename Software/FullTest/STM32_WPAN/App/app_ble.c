...
static const char a_LocalName[] = {AD_TYPE_COMPLETE_LOCAL_NAME, 'H','I','D','M','o','u','s','e'};
...
void APP_BLE_Init(void)
{
   ...
   */
  printf("Calling HIDSAPP_Init...\r\n");
  HIDSAPP_Init();
  printf("HIDSAPP_Init done\r\n");
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
      {
          ...
          if(index < CFG_MAX_CONNECTION)
          {
              ...
              /* Request security - required for HID on Windows */
              //aci_gap_slave_security_req(BleApplicationContext.BleApplicationContext_legacy.connectionHandle[index]);
              printf("Security requested\r\n");
          }
          else
          {
              printf("No free connection slot!\r\n");
          }
      }
      break;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
        ...
        {
            case ACI_GAP_BOND_LOST_VSEVT_CODE:      // 0x0405
                printf("Bond lost\r\n");
                aci_gap_allow_rebond(
                    BleApplicationContext.BleApplicationContext_legacy.connectionHandle[0]);
                break;

            case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:  // 0x0401
                printf("Pairing complete\r\n");
                break;

            case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:  // 0x0C01
            {
                aci_gatt_attribute_modified_event_rp0 *pr =
                    (aci_gatt_attribute_modified_event_rp0*)p_blecore_evt->data;
                break;
            }

            case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:  // 0x0C13
            {
                aci_gatt_write_permit_req_event_rp0 *pr =
                    (aci_gatt_write_permit_req_event_rp0*)p_blecore_evt->data;
                aci_gatt_write_resp(
                    BleApplicationContext.BleApplicationContext_legacy.connectionHandle[0],
                    pr->Attribute_Handle, 0x00, 0x00, pr->Data_Length, pr->Data);
                break;
            }
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  ret = aci_hal_write_config_data(CONFIG_DATA_RANDOM_ADDRESS_OFFSET,
                                  ...
                                  (uint8_t*) p_bd_addr);  if (ret != BLE_STATUS_SUCCESS)
  {
